...
export async function consult(history, opts = {}) {
  ...
  const generateOnce = async (effort) => {
    const response = await client.messages.create({
      model: MODEL,
      max_tokens: 16000, // adaptive思考＋JSON出力の枯渇・途中崩壊を防ぐ余裕（思考で食い切らないよう増量）
    ...
    // ★途中打ち切り・拒否を検知（劣化JSON＝「br」等の主因）。throwして再生成へ回す。
    if (response.stop_reason === "max_tokens")
      throw new Error("truncated: stop_reason=max_tokens (JSON不完全)");
    if (response.stop_reason === "refusal")
      throw new Error("model refused: stop_reason=refusal");
