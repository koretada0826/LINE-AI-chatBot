...
// 「今すぐ相談」ボタン用の"軽い"カード（通常）。命のホットラインは出さず、
// 運営（社外）への即つなぎを、やわらかく案内する。※命の危機ワード検知時だけ emergencyFlex() を使う。
export function urgentLightFlex() {
  const BLUE = "#2F6FB0";
  return {
    type: "flex",
    altText: "今すぐ相談",
    quickReply: navQuickReply(), // 行き止まりにしない（チャット/メンター/終える へ戻れる）
    contents: {
      type: "bubble",
      size: "mega",
      header: {
        ...
        contents: [
          ...
          { type: "text", text: "急ぎで話したいときは、運営（社外）の担当にすぐおつなぎします。", color: "#EAF2FB", size: "sm", wrap: true, margin: "sm" },
        ],
      },
      body: {
        type: "box",
        ...
        paddingAll: "18px",
        contents: [
          { type: "text", text: "「すぐに誰かに聞いてほしい」ときは、下のボタンで運営（社外の相談窓口）に連絡します。日中はできるだけ早く、夜間・休日は順番にご連絡しますね。", size: "sm", color: "#333333", wrap: true },
          { type: "button", style: "primary", color: BLUE, height: "md", margin: "md", action: { type: "postback", label: "運営（社外）に今すぐつないでもらう", data: "urgent_connect", displayText: "今すぐ運営につないでほしい" } },
          { type: "text", text: "このまま、ここでチャットを続けても大丈夫です。", size: "xxs", color: "#8894A3", align: "center", margin: "sm" },
        ],
      },
    },
  };
...
// 命に関わる危機のときだけ使う"重い"カード（命のホットライン等）。detectLifeCrisis時のみ。
