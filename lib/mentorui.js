...
// メッセージ下部に出す共通クイックリプライ（🗣️人と話したい ／ ⚡今すぐ相談したい）
export function humanQuickReply() {
  return {
    items: [
      { type: "action", action: { type: "postback", label: "🗣️ 人と話したい", data: "want_human", displayText: "人と話したい" } },
      { type: "action", action: { type: "postback", label: "⚡ 今すぐ相談したい", data: "want_now", displayText: "今すぐ相談したい" } },
    ],
  };
}
...
export function mentorWelcome(mentors) {
  ...
  const carousel = mentorCarousel(mentors, { urgent: false });
  // カード下部に「今すぐ相談したい」等のボタンを常設（登録直後から押せる）
