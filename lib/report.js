...
export function reportToHtml(rep, companyName = "", periodLabel = "全期間") {
  const catRows = rep.byCategory
    .map(
      (c) => `<tr><td>${he(c.name)}</td><td class="num">${c.count}</td>
  ...
  const summary = rep.total
    ? `この期間、最も多かったのは <b>${top3 || "—"}</b> でした。`
    : "この期間の相談はまだありません。";
...
<style>
  *{box-sizing:border-box} body{font-family:-apple-system,'Hiragino Sans',sans-serif;margin:0;background:#f4f5f7;color:#1c2430}
  ...
  .summary{background:#eef3f8;border-radius:12px;padding:14px 16px;font-size:15px;line-height:1.8;margin-bottom:18px}
  ...
  <div class="cards">
    <div class="card"><div class="n">${rep.total}</div><div class="l">相談件数</div></div>
