...
<style>
  ...
  *{box-sizing:border-box}
  ...
  <button class="printbtn" onclick="window.print()">🖨 PDFで保存（1枚に収めて共有）</button>
  <div class="page">
    <header class="cover">
  ...
  <script>
    // 印刷時、用紙サイズを"中身の高さ"ぴったりにして、常に1枚で出力する
    function onePage(){
      var el=document.querySelector('.page'); if(!el) return;
      var px2mm=25.4/96;
      var w=Math.ceil(el.offsetWidth*px2mm);
      var h=Math.ceil(el.scrollHeight*px2mm)+2;
      var st=document.getElementById('ps')||document.createElement('style');
      st.id='ps'; st.textContent='@page{size:'+w+'mm '+h+'mm;margin:0}';
      document.head.appendChild(st);
    }
    window.addEventListener('load',onePage);
    window.addEventListener('beforeprint',onePage);
  </script>
</body></html>`;
}
  ...
  const bodyRows = sorted
    .map((r) => {
      ...
      return `<tr>
        ...
        <td class="act"><a href="${link}">レポート →</a><br><a href="/api/people?token=${tk}&company_id=${r.id}${sinceParam}" style="color:#8a3a3a">👤 個人ログ →</a></td>
      </tr>`;
    })
    .join("");
  const d = new Date();
...
// ============================================================
// 運営専用：個人を特定した相談ログ（誰が・どんな相談を・何件）
// ★このデータは運営(Uniboost)だけが見られる。企業には絶対に出さない。
// consultation_logs を employees(氏名・部署・役職)と line_user_id で突き合わせて人物特定する。
// ============================================================
export async function generatePeopleLogs(companyId, sinceDays = null) {
  if (!supabase || !companyId) return null;
  let q = supabase
    .from("consultation_logs")
    .select("line_user_id, category, topic, summary, risk_level, escalated, department, created_at")
  ...
  if (error) {
    console.error("generatePeopleLogs error:", error.message);
    return null;
  }
  const { data: emps } = await supabase
    .from("employees")
    .select("line_user_id, name, department, role_title")
    .eq("company_id", companyId);
  const empMap = {};
  for (const e of emps || []) empMap[e.line_user_id] = e;

  const byPerson = {};
  for (const l of logs || []) {
    const key = l.line_user_id || "(不明)";
    if (!byPerson[key]) {
      const e = empMap[key] || {};
      byPerson[key] = {
        name: e.name || "(氏名未登録)",
        department: e.department || l.department || "",
        role: e.role_title || "",
        count: 0,
        lastAt: l.created_at,
        logs: [],
    ...
    byPerson[key].logs.push(l);
  }
  const people = Object.values(byPerson).sort((a, b) => b.count - a.count);
  return { companyId, total: (logs || []).length, people };
}

const RISK_LABEL = { 0: "", 1: "軽", 2: "中", 3: "高" };
export function peopleLogsHtml(rep, companyName = "", periodLabel = "全期間", masterToken = "") {
  const tk = encodeURIComponent(masterToken);
  const d = new Date();
  const dstr = `${d.getFullYear()}/${d.getMonth() + 1}/${d.getDate()}`;
  const personCards = (rep.people || [])
    .map((p) => {
      const logRows = p.logs
        .map((l) => {
          const dt = new Date(l.created_at);
          const ds = `${dt.getMonth() + 1}/${dt.getDate()} ${String(dt.getHours()).padStart(2, "0")}:${String(dt.getMinutes()).padStart(2, "0")}`;
          const risk = RISK_LABEL[l.risk_level] || "";
          const esc = l.escalated ? '<span class="esc">要対応</span>' : "";
          return `<tr>
            <td class="dt">${ds}</td>
            <td>${he(l.topic || "-")} <span class="cat">${he(catLabel(l.category))}</span> ${risk ? `<span class="risk r${l.risk_level}">${risk}</span>` : ""} ${esc}</td>
          </tr>
          <tr class="sumrow"><td></td><td class="sum">${he(l.summary || "")}</td></tr>`;
        })
        .join("");
      return `<section class="person">
        <div class="phead">
          <div class="pname">${he(p.name)}${p.role ? `<span class="prole">${he(p.role)}</span>` : ""}${p.department ? `<span class="pdept">${he(p.department)}</span>` : ""}</div>
          <div class="pcount">相談 ${p.count} 件</div>
        </div>
        <table>${logRows}</table>
      </section>`;
    })
  ...
  return `<!doctype html><html lang="ja"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>相談ログ（運営専用） ｜ ${he(companyName)}</title>
<style>
  *{box-sizing:border-box} body{font-family:-apple-system,'Hiragino Sans',sans-serif;margin:0;background:#f4f5f7;color:#1c2430}
  .wrap{max-width:820px;margin:0 auto;padding:24px 18px 60px}
  .head{background:#3a1c1c;color:#fff;border-radius:16px;padding:22px 20px}
  .head h1{font-size:18px;margin:0 0 6px} .head .sub{opacity:.85;font-size:13px}
  .warn{background:#fff4e5;border:1px solid #f0c98a;color:#8a5a1a;border-radius:12px;padding:12px 14px;font-size:12.5px;line-height:1.7;margin:14px 0}
  .filters{margin:0 0 12px;font-size:13px}
  .filters a{display:inline-block;padding:6px 12px;margin-right:8px;border-radius:20px;background:#eef1f4;color:#2f6fb0;text-decoration:none}
  .filters a.on{background:#2f6fb0;color:#fff}
  .person{background:#fff;border-radius:14px;padding:14px 16px;margin-bottom:14px;box-shadow:0 1px 3px rgba(0,0,0,.05)}
  .phead{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;border-bottom:1px solid #eef1f4;padding-bottom:8px}
  .pname{font-size:15px;font-weight:700;color:#0e2a47}
  .prole,.pdept{font-size:11px;font-weight:600;color:#667;background:#eef1f4;border-radius:10px;padding:2px 8px;margin-left:8px}
  .pcount{font-size:12px;color:#8894a3;font-weight:700}
  table{width:100%;border-collapse:collapse}
  td{padding:6px 4px;font-size:13.5px;vertical-align:top}
  .dt{width:74px;color:#8894a3;font-size:12px;white-space:nowrap}
  .cat{font-size:11px;color:#2f6fb0;background:#eef4fb;border-radius:8px;padding:1px 7px;margin-left:4px}
  .risk{font-size:11px;font-weight:700;border-radius:8px;padding:1px 7px;margin-left:4px}
  .risk.r1{color:#8a6d1a;background:#fbf3d5} .risk.r2{color:#a15c00;background:#ffe6c7} .risk.r3{color:#fff;background:#c0392b}
  .esc{font-size:11px;font-weight:700;color:#fff;background:#c0392b;border-radius:8px;padding:1px 7px;margin-left:4px}
  .sumrow .sum{color:#556;font-size:12.5px;line-height:1.6;padding-bottom:10px;border-bottom:1px solid #f4f6f8}
  .foot{color:#98a3b0;font-size:12px;text-align:center;margin-top:20px}
</style></head><body><div class="wrap">
  <div class="head"><h1>相談ログ（運営専用・個人特定）</h1>
    <div class="sub">${he(companyName)}　対象：${he(periodLabel)}　閲覧日：${dstr}　相談 ${rep.total} 件</div></div>
  <div class="warn">⚠️ この画面は<b>運営（Uniboost）専用</b>です。氏名など個人が特定できる情報を含みます。<b>企業（クライアント）には絶対に共有されません</b>。取り扱い・画面共有に十分ご注意ください。</div>
  <div class="filters">
    期間：
    <a href="/api/people?token=${tk}&company_id=${rep.companyId}&since=7">直近7日</a>
    <a href="/api/people?token=${tk}&company_id=${rep.companyId}&since=30">直近30日</a>
    <a href="/api/people?token=${tk}&company_id=${rep.companyId}">全期間</a>
    <a href="/api/report?token=${tk}">← 運営ダッシュボードへ</a>
  </div>
  ${personCards || '<section class="person"><div class="pname">この期間の相談はありません</div></section>'}
  <div class="foot">才職CARE ｜ 運営コンソール（Uniboost）</div>
</div></body></html>`;
}
