...
const STYLE = `
  *{box-sizing:border-box} body{font-family:-apple-system,'Hiragino Sans',sans-serif;margin:0;background:#f4f5f7;color:#1c2430}
  ...
  .printbtn{position:fixed;right:18px;bottom:18px;z-index:9;padding:12px 18px;border:0;border-radius:24px;
    background:#0e2a47;color:#fff;font-size:14px;font-weight:700;box-shadow:0 4px 12px rgba(0,0,0,.18);cursor:pointer}
  @media print{
    body{background:#fff} .wrap{max-width:100%;padding:0}
    .printbtn,.filters{display:none} section{box-shadow:none;border:1px solid #e4e8ec}
...
<title>請求書 ｜ ${he(b.companyName)}</title><style>${STYLE}</style></head><body>
  <button class="printbtn" onclick="window.print()">🖨 請求書をPDFで保存</button>
  <div class="wrap">
