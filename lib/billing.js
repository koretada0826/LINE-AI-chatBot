...
// 優先度別の時間単価（No.10 チャット稼働などの手入力の参考値）
...
export async function computeBilling(company, month, headcount = null) {
  ...
  const total = fixedTotal + itemsTotal; // 当月の合計請求額
...
<title>請求サマリー ｜ ${he(b.companyName)}</title><style>${STYLE}</style></head><body><div class="wrap">
  ...
  <section>
    <h2>② セッション従量（面談・レポート・研修 等）</h2>
    <table>
      <thead><tr><th>メニュー</th><th class="num">数量</th><th class="num">単価</th><th class="num">金額</th></tr></thead>
      <tbody>${itemRows}
        <tr class="total"><td>従量小計</td><td></td><td></td><td class="num">${yen(b.itemsTotal)}</td></tr>
      </tbody>
    </table>
  </section>
  <section>
    <table><tbody>
      <tr class="total"><td>当月 合計請求額</td><td></td><td></td><td class="num">${yen(b.total)}</td></tr>
    </tbody></table>
  </section>
  <p class="note">※固定部分は登録社員数から自動算出（月額基本料＋社員数×単価）。セッション従量はメニュー明細の合算です。<br>初期費用（社長・人事インタビュー等）は導入時1回のため、この月次請求には含みません。</p>
  ${backHref ? `<p class="act"><a href="${backHref}">← 一覧に戻る</a></p>` : ""}
  <div class="foot">才職CARE ｜ 請求（Uniboost）</div>
</div></body></html>`;
}
...
export function billingAdminHtml(rows, month, masterToken = "") {
  ...
  const sumItems = rows.reduce((s, r) => s + r.itemsTotal, 0);
  ...
  const body = sorted
    .map(
      ...
      <td class="num">${yen(r.itemsTotal)}</td>
...
<title>請求一覧（運営） ｜ 才職CARE</title><style>${STYLE}</style></head><body><div class="wrap">
  ...
  <section>
    <div class="filters"><a class="act" href="/api/report?token=${tk}">← 運営ダッシュボードへ</a></div>
    <table>
      ...
      <tbody>${body || '<tr><td colspan="6" class="muted">企業がありません</td></tr>'}
        <tr class="total"><td>全社合計</td><td class="num">${sumHead}</td><td class="num">${yen(sumFixed)}</td><td class="num">${yen(sumItems)}</td><td class="num">${yen(sumTotal)}</td><td></td></tr>
      </tbody>
    </table>
  </section>
  <p class="note">※固定＝月額基本料＋登録社員数×単価（自動）。従量＝当月のセッション明細（面談・レポート等）の合算。<br>従量明細は現在 billing_items に手入力（or 将来Spir/稼働ログ連携）で積み上げます。初期費用は月次に含みません。</p>
