...
// ============================================================
import { getEmployee, upsertEmployee, searchCompanies, getCompany } from "./tenant.js";
...
export async function handleOnboarding(userId, event) {
      ...
      messages: [
        {
          ...
          text:
            ...
            "ご相談の前に、かんたんな登録をお願いします（1分ほど）。上司や人事に伝わることはないので、安心してくださいね。\n" +
            "下のボタンから登録フォームを開いてください👇\n" +
            "（フォームが開けない場合は、このままメッセージでも登録できます。お勤めの会社名を入力してください）",
          quickReply: {
            items: [
              {
                type: "action",
  ...
  if (postbackData.startsWith("company:")) {
    ...
    return {
      messages: [msg(`「${company.name}」ですね。ありがとうございます。\n次に、お名前（氏名）を教えてください。`)],
      registered: false,
    };
  }
  ...
  switch (step) {
    case "company": {
      if (!text) return { messages: [msg("お勤めの会社名を入力してください。（社名の一部でOK）")], registered: false };
      const companies = await searchCompanies(text);
      if (!companies.length) {
        return {
          messages: [
            msg(
              "その会社が見つかりませんでした🙏 正式名称の一部、または運営から共有された「企業コード」を入力してください。分からない場合は、担当者にご確認ください。"
            ),
          ],
          registered: false,
        };
      ...
      return {
        messages: [
          msg(
            "会社を選んでください👇",
            companies.map((c) => ({ label: c.name, data: `company:${c.id}` }))
          ),
        ],
        registered: false,
      };
    ...
    default:
      // 想定外：登録済み扱いにフォールバック
      await upsertEmployee(userId, { onboarding_step: "done", registered: true });
      return { messages: [doneMessage()], registered: true };
  }
}
