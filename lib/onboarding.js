...
function afterCompanyMessage(uid, company) {
  return formPrompt(uid, {
    ...
    heading: `「${company.name}」ですね😊 ありがとうございます。`,
    lines: [
      "残りの登録（氏名・役職・電話・メール）は、下のリンクから1画面でまとめてお願いします。",
      "（企業番号は入力済みです）",
    ],
    note: "リンクが使えないときは、このまま順番にお答えいただいてもOK。まずはお名前から。",
  });
}
...
export async function handleOnboarding(userId, event) {
  ...
  if (!emp) {
    ...
    return {
      messages: [
        formPrompt(userId, {
          heading: "はじめまして😊 来てくださって、ありがとうございます。",
          lines: [
            "ご相談の前に、かんたんな登録をお願いします（1分ほど）。",
            "上司や人事に伝わることはないので、安心してくださいね。",
          ],
          note: "リンクが使えない場合は、会社から配布された「企業番号」をこのまま送信してもOKです。",
        }),
      ],
      registered: false,
    };
