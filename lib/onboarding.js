...
function afterCompanyMessage(uid, company) {
  return {
    type: "text",
    text:
      `「${company.name}」ですね、ありがとうございます😊\n` +
      `残りの登録（氏名・役職・電話・メール）は、下のフォームで一度にまとめてお願いします👇\n` +
      `（企業番号は入力済みです。フォームが使えないときは、このまま順番にお答えいただいてもOK。まずはお名前から）`,
    quickReply: {
      items: [
        {
          type: "action",
          action: {
            type: "uri",
            label: "📝 登録フォームを開く",
            uri: formUrl(uid, company.invite_code),
          },
        },
      ],
    },
      ...
      messages: [
        {
          ...
          text:
            ...
            "（フォームが開けない場合は、このままメッセージでも登録できます。会社から配布された「企業番号」を入力してください）",
          quickReply: {
            items: [
              {
                type: "action",
