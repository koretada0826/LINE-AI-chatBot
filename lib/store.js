...
export async function logConsultation(userId, result, companyId = null) {
...
export async function getUserProfile(userId) {
  if (!supabase) return memUserProfile.get(userId) ?? { profile: null, updated_at: null };
  const { data, error } = await supabase
    .from("user_profiles")
  ...
  return { profile: data?.profile ?? null, updated_at: data?.updated_at ?? null };
}
...
export async function saveUserProfile(userId, profile) {
  ...
  if (!supabase) {
    memUserProfile.set(userId, { profile, updated_at: now });
    return;
  }
  const { error } = await supabase
    .from("user_profiles")
