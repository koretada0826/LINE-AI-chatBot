...
export async function getUserProfile(userId) {
  if (!supabase) return memUserProfile.get(userId) ?? { profile: null, updated_at: null, support_style: null, pending_feedback: null };
  const { data, error } = await supabase
    .from("user_profiles")
    .select("profile, updated_at, support_style, pending_feedback")
    .eq("line_user_id", userId)
    .maybeSingle();
  if (error) {
    console.error("getUserProfile error:", error.message);
    return { profile: null, updated_at: null, support_style: null, pending_feedback: null };
  }
  return {
    profile: data?.profile ?? null,
    updated_at: data?.updated_at ?? null,
    support_style: data?.support_style ?? null,
    pending_feedback: data?.pending_feedback ?? null,
  };
}

// 会話後アンケート：1〜5評価を保存し、「理由の入力待ち」状態にする（次の発言を理由として拾う）
export async function saveFeedbackRating(userId, rating, companyId = null) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("feedback")
    .insert({ line_user_id: userId, company_id: companyId, label: "rating", rating })
    .select("id")
    .maybeSingle();
  if (error) { console.error("saveFeedbackRating error:", error.message); return null; }
  const fbId = data?.id ?? null;
  await supabase
    .from("user_profiles")
    .upsert({ line_user_id: userId, pending_feedback: { id: fbId, rating } }, { onConflict: "line_user_id" });
  return fbId;
}

// 評価の理由を、直前の評価レコードに追記し、待ち状態を解除する
export async function saveFeedbackReason(userId, pending, reason) {
  if (!supabase) return;
  if (pending?.id) {
    const { error } = await supabase
      .from("feedback")
      .update({ reason: (reason || "").slice(0, 1000) })
      .eq("id", pending.id);
    if (error) console.error("saveFeedbackReason error:", error.message);
  }
  await clearPendingFeedback(userId);
}

// 「理由の入力待ち」状態を解除
export async function clearPendingFeedback(userId) {
  if (!supabase) return;
  const { error } = await supabase
    .from("user_profiles")
    .upsert({ line_user_id: userId, pending_feedback: null }, { onConflict: "line_user_id" });
  if (error) console.error("clearPendingFeedback error:", error.message);
}
